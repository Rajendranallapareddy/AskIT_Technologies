...
// Renders the marketing chrome around public pages.
// Dashboard pages use DashboardLayout separately.
...
export default function App() {
  ...
  // Handles:
  // 1. AskIT real-time/in-app notifications
  // 2. Mobile/browser Web Push registration
  useEffect(() => {
    if (
      isAuthenticated &&
      user
    ) {
      // Existing real-time notification connection
      initNotifications(
        ...
        async () => {
          try {
            if (
              ...
              console.log(
                '[PUSH] Push notifications are not supported on this device.'
              );
            ...
            await initialisePushServiceWorker();

            const permission =
              getPushPermission();

            // Already allowed:
            // automatically register/update the device.
            if (
              permission ===
              'granted'
            ) {
              ...
              console.log(
                '[PUSH] AskIT mobile notifications are active.'
              );
            } else if (
              permission ===
              'default'
            ) {
              // The PushPermissionPrompt component
              // will ask the user through a proper
              // user interaction.
              console.log(
                '[PUSH] Notification permission has not been requested yet.'
              );
            } else {
              console.log(
                '[PUSH] Notifications are blocked on this device.'
              );
            }
          } catch (error) {
            console.error(
              '[PUSH] Unable to initialise push notifications:',
              error
            );
          }
        };
  ...
  return (
    <>
      ...
      {/* One-time mobile notification permission prompt */}
      {isAuthenticated &&
        user && (
