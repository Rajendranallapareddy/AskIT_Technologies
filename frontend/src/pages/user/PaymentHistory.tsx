...
export default function PaymentHistory() {
  ...
  const downloadReceipt =
    async (paymentId: string) => {
      ...
      try {
        const response: any =
          await paymentApi.downloadReceipt(
            paymentId
          );
         ...
         *
         * and API wrappers that already unwrap response.data:
        ...
        if (!fileUrl) {
          throw new Error(
            'Receipt download URL was not returned by the server.'
          );
        }
        ...
        /**
         * Some browsers can block window.open()
         * because the call happens after awaiting an API request.
         *
         * If that happens, redirect the current tab instead.
         */
        if (!newWindow) {
      ...
      onSuccess: (result) => {
        toast.success(
          result.pendingApproval
            ? 'Payment received! Pending Super Admin approval.'
            : 'Payment successful!'
        );
    ...
    >
      {isLoading || !data ? (
        ...
        <EmptyState
          ...
          description="Payments for paid internships will appear here."
        />
      ) : (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-extrabold text-navy-900 leading-tight">
                      {formatMoney(
                        p.totalAmount
                      )}
                    </p>
                ...
                {p.status ===
                  ...
                  p.receipt && (
                    <div className="mt-4 pt-4 border-t border-navy-50 flex flex-wrap gap-2">
                      ...
                      <Button
                        variant="ghost"
                        className="!py-2 text-xs !text-navy-600 !border-navy-200"
                        icon={
                ...
                {isPending && (
                  <div className="mt-4 pt-4 border-t border-navy-50">
                    ...
                    <p className="text-xs text-amber-600 mb-2">
                      {isInstallment
                        ? 'This installment is still due.'
                        : 'Payment is still pending — pay online now, or scan a UPI QR code / pay by bank transfer.'}
                    </p>
      ...
      >
        <div className="space-y-4">
          ...
          <p className="text-sm text-navy-600">
            ...
            (
            {formatMoney(
              refundTarget?.totalAmount
            )}
            ). An admin will review and
